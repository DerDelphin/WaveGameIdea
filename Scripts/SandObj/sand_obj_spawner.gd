...
@onready var SandObj:PackedScene = preload("res://Scenes/SandObj/sand_object.tscn")
@onready var StrongSandObj:PackedScene = preload("res://Scenes/SandObj/strong_sand_object.tscn")
@onready var builderCastle: PackedScene = preload("res://Scenes/SandObj/builder_castle.tscn")
@onready var timer: Timer = $Timer
var spawnedNum: int = 0

func _ready() -> void:
	timer.wait_time = DayInfoManager.getSpawnDuration(GLOBAL.dayCount-1)
	GLOBAL.newDayStarted.connect(
		func():timer.wait_time = DayInfoManager.getSpawnDuration(GLOBAL.dayCount-1))
...
func spawn() -> void:
	...
	var instance: Node
	if(GLOBAL.Chance(DayInfoManager.getStrongSpawnChance(GLOBAL.dayCount-1))):
...
func maybeSpawnBuilderCastle()-> void:
	var spawnChance: float = DayInfoManager.getBuilderSpawnChance(GLOBAL.dayCount-1)
	if spawnChance == 0:
		return
